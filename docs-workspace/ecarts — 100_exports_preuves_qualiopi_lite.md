...
> **Mise à jour post-cluster 16 — 2026-06-18** · **Cluster 16 :** pas d'impact direct. **Confirmé local :** exports tenant-scoped (D1-02).
...
### 4.6 Décision lot courant — bundles Qualiopi et EvaluationTrace (D2-M05)

**Référence :** `mapping_EvaluationTrace_runtime_local.md` §1.6 et §3.

| Question | Réponse lot courant |
|----------|---------------------|
| `EvidenceBundleView` produit-il une EvaluationTrace 2.0 ? | **Non** — ZIP audit avec `traces.json` (métadonnées), `audit_log.json`, `referentials.json`, `active_documents.json`, `summary.txt` |
| Le bundle est-il certifiant ? | **Non** — export Qualiopi **lite**, non certifiant (doctrine inchangée) |
| Lien avec `LearnerEvaluationRecord` ? | **Absent du ZIP local** — évaluations exportables via `trainer/evaluation-records/export/` séparément |
| Lien avec `EvaluationTrace` doctrinal ? | **CIBLE** — alignement complet agrégat ↔ bundle = chantier futur ; vocabulaire pont documenté |
| Export `trace_rich_v1` (ExportRun) | **RÉEL OBSERVÉ** — JSON avec `payload_structured` par trace ; distinct du bundle Qualiopi |

**A_VÉRIFIER (Encoors) :** contenu exact des ZIP prod ; RLS sur accès bundle ; éventuels fichiers additionnels non observés en local.

**Garde-fou :** ne pas présenter le bundle comme preuve d’une EvaluationTrace 2.0 complète tant que l’audit prod n’a pas confirmé la richesse effective.
...
| Mapping entre EvaluationTrace cible et objets réels d’évaluation/export | Agrégat doctrinal ; pas d’objet unique en base. | Mapping dans `mapping_EvaluationTrace_runtime_local.md` (D2-M05 doc livré). | Dispersion **nommée** ; enrichissement bundle/agrégat = CIBLE. | **RENOMMER_DANS_DOC** | Référencer le mapping ; ne pas lisser la friction résiduelle. |
...
| EXP-007 | P1 | ~~Produire un tableau de mapping entre `EvaluationTrace` cible et objets réels~~ | Documentation | **Fait** — `mapping_EvaluationTrace_runtime_local.md` (D2-M05) | — | Enrichissement bundle / agrégat runtime reste backlog séparé |
...
**Sources :** `cluster_dev_encadrants_exports_vague2_resultats.md` · tests `test_evidence_exif.py`, `test_d2_m06_d2_m11_exports_and_analytics.py`.

| Point | Avant cluster | Après cluster (RÉEL OBSERVÉ) |
|---|---|---|
| EXIF suppression | Docstring seule | **Implémenté** (`evidence_media.py`) ; meta `exif_stripped: true` |
| GPS opt-in | Meta seulement | **Confirmé** — GPS en meta explicite, pas dans fichier image |
| ExportRun / EvidenceBundle rôles | Backend ORGADMIN/SUPERADMIN ; front trop permissif | **Aligné** front + tests négatifs TUTOR/TRAINER/LEARNER |
| Bundle Qualiopi lite | Métadonnées seulement | **Inchangé** — toujours non certifiant, non EvaluationTrace 2.0 |
| Cross-tenant bundle | ExportRun testé | **+ test** `test_evidence_bundle_cross_tenant` |
| D9bis / export-md | Absent | **Couronne** — non traité |
| Prod Encoors ZIP | — | **A_VÉRIFIER** |

---

## 9. Mise à jour cluster dev vague 3 — trace_rich_v1 et pivot (réel local)

**Sources :** `cluster_dev_fin_de_fil_qualite_observabilite_vague3_resultats.md` · `exports/views.py` · tests `test_evaluation_trace_minimal.py`.

| Point | Après cluster (RÉEL OBSERVÉ) |
|---|---|
| ExportRun JSON | Chaque trace inclut `evaluation_trace_pivot_v1` (session, record, trace, evidence, validation humaine) |
| EvidenceBundle lite | **Inchangé** — métadonnées traces seulement, pas de pivot ni payload_structured |
| D9bis / LLM analysis | **Absent** — contrats CIBLE dans `d9bis_contracts.py` ; tests absence confirmés |
| Bundle certifiant / EvaluationTrace 2.0 | **Non** — disclaimer pivot + bundle lite maintenu |

**A_VÉRIFIER :** contenu ExportRun JSON prod Encoors.

---

## 10. Mise à jour cluster dev vague 4 — canal D9bis séparé

| Point | Statut |
|---|---|
| ExportRun / EvidenceBundle / CSV Qualiopi | **Sans D9bis** — confirmé tests absence |
| Export technique D9bis | Canal séparé `d9bis_session_export_v1` — SUPERADMIN only |
| trace_rich_v1 / pivot | **Sans analytics LLM** |
| Bundle certifiant | **Non** — inchangé |

---

## 11. Mise à jour cluster audit vague 5 — E2E exports

| Point | Statut vague 5 |
|---|---|
| ExportRun JSON + pivot | **ALIGNE local** — scénarios O1/EVAL1 |
| EvidenceBundle lite | **ALIGNE local** |
| Exports sans D9bis | **ALIGNE** |
| Encoors ExportRun/bundle payload | **A_VÉRIFIER** |

---

## 12. Mise à jour cluster 8 OPS — exports UI

| Point | Statut cluster 8 |
|---|---|
| CTA exports ORGADMIN navigateur | **ALIGNE** — Playwright smoke |
| LEARNER sans CTA exports | **ALIGNE** — smoke combiné |
| `export_run` RLS enabled | **ALIGNE** local pytest |
| Encoors exports authentifiés | **A_VÉRIFIER** |

---

## 13. Mise à jour cluster 11 — parité ExportRun Encoors

**Sources :** `cluster11_verrouillage_runtime_vs_encoors_resultats.md` · oracle O1.

| Point | Statut cluster 11 |
|---|---|
| ExportRun + pivot local | **ALIGNE** |
| EvidenceBundle lite local | **ALIGNE** |
| Encoors O1 authentifié | **A_VÉRIFIER** — protocole prêt |
| Taxonomie E1–E6 | **COURONNE** DOC |

---

## 14. Mise à jour cluster 14 — exports côté encadrants

**Sources :** `cluster14_interfaces_encadrants_observabilite_plan_resultats.md` · `design_D2-M06_D2-M11_exports_analytics.md`.

| Cat. | UI encadrant observée | Statut |
|---|---|---|
| E2 evaluation-records export | API TRAINER/ORGADMIN — pas bouton prod | **ALIGNE API** |
| E3 EvidenceBundle | **A_VÉRIFIER** bouton UI ; API OK | **PARTIEL** |
| E4 ExportRun CSV/JSON | `GroupAdminDetailView` tester | **ALIGNE local** |
| E5 debug | Tester/internal only | **ALIGNE** |
| E6 D9bis | Absent | **COURONNE** |
| Taxonomie E1–E6 ↔ écrans | **DOC cluster 14** | **CONSOLIDÉ** |
