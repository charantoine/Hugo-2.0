...
| `VITE_LEARNER_UI_V2` | `true` | Layout apprenant **v2** (fil + sidebar) vs legacy |

**Bascule layout** (`src/App.vue` L9–11) : `route.meta.layout === 'tester'` → `TesterLayout`, sinon `ProdLearnerLayout`.

### 2.1 Interface apprenant v2 (affichage — juin 2026)

**Statut :** **IMPLÉMENTÉ** côté front uniquement ; **pas** de changement contrat API / UIState.

| Élément | Réel observé |
|---------|--------------|
| Flag | `VITE_LEARNER_UI_V2` → `frontendFeatures.learner_ui_v2` (défaut `true`) |
| Composants | `LearnerConversationFeed.vue`, `LearnerActionSidebar.vue` ; `ProdLearnerWorkspace.vue` double layout |
| UX v2 | Fil conversation central ; barre droite (postures + synthèse + évaluation + traces) ; sélecteurs groupe/profil masqués ; récompenses masquées ; progression sous le fil |
| Postures | `PostureSelector` variante `buttons` — Diag / Réflexif / Bûchage (`conversationPostures.js`) |
| Désactivation | `VITE_LEARNER_UI_V2=false` → layout legacy |

**Cible 2.0 :** rapprochement spec interface apprenant — **PARTIEL** (chrome UX, pas moteur).

### 2.2 Bibliothèque formateur — fiabilité documentaire (juin 2026)

**Statut :** **PARTIEL+** local.

| Élément | Réel observé |
|---------|--------------|
| Surface | `ProdTrainerLibraryView.vue` — édition inline `trainer_reliability` dans `document.meta` |
| Réindex | Réindexation auto déclenchée après PATCH fiabilité (évite chunks stale) |
| Scoring RAG | Niveau risque influencé par fiabilité quand RAG actif (`apps/library/document_meta.py`) |
| Tests | `test_reliability_rag_and_reindex.py` ; E2E `trainer_library_reliability.spec.ts` (TLIB-REL-01/02 PASS) |
| ACL groupes TRAINER | `GET /groups/` — tous les groupes org (correction juin 2026, `views_groups.py`) |

**A_VÉRIFIER :** Encoors ; gate RAG AFEST (`TLIB-RAG-01` — échec connu).

### 2.3 Navigation formateur — shell prod (juin 2026, post-push 20/06)

**Statut :** **IMPLÉMENTÉ** local — correction UX navigation formateur.

| Élément | Réel observé | Fichier |
|---------|--------------|---------|
| **Point d'entrée TRAINER** (login) | `/app/trainer/knowledge` — **plus** `/dashboard` par défaut | `frontendConfig.js` → `resolveAuthenticatedHome()` ; `LoginView.vue` ; garde `router/index.js` si accès `/dashboard` |
| Hub formateur | Orchestrateur de connaissance + liens vers bibliothèque, référentiels, élicitation | `ProdTrainerKnowledgeView.vue` |
| Routes prod formateur | `/app/trainer` (redirect → knowledge), `/app/trainer/knowledge`, `/app/trainer/library`, `/app/trainer/elicitation`, `/app/trainer/referentials`, `/app/trainer/referentials/import` | `router/index.js` |
| Layout | `ProdLearnerLayout` (`meta.layout: 'prod'`) | `App.vue` |
| Topbar — espace métier | Lien **« Espace formateur »** → `/app/trainer/knowledge` | `ProdLearnerLayout.vue` |
| Topbar — chat apprenant | Lien **« Chat apprenant »** (ex-« Mon espace ») → `/app` ; placé **à droite** (`prod-topbar__actions`) | idem |
| Référentiels (liste + import) | Accessibles depuis le hub formateur, **même shell prod** (réutilise `ReferentialListView` / `ReferentialImportView`) | routes `ProdTrainerReferentials*` |
| Retour hub formateur | Bouton **« Retour orchestrateur »** sur `/app/trainer/library`, `/app/trainer/elicitation`, `/app/trainer/referentials` → `/app/trainer/knowledge` | `TrainerBackToOrchestratorLink.vue` |
| `/dashboard` pour TRAINER | **Legacy testeur** — calibration encadrant ; **n'est plus** le redirect post-login | `DashboardView.vue` ; tuteur/coordo/orgadmin inchangés |

**Limite connue (non migrée) :** la configuration référentiel **par groupe** (`/group/:groupId/referential`, `GroupReferentialConfigView`) reste en **layout testeur** — pas d'équivalent `/app/trainer/group/:id/referential` en prod. Point ouvert, pas comportement cible final.

**Cible 2.0 :** hub formateur unifié — **PARTIEL+** (entrée + référentiels globaux en prod ; config groupe encore testeur).
...
| `layouts/ProdLearnerLayout.vue` (topbar) | `Hugo - Lucia` / « Parcours apprenant » ; **« Chat apprenant »** (droite) ; **« Espace formateur »** si TRAINER |
...
> **Corrigé / complété par [`DOC_RUNTIME_UPDATE_2026-06.md`](DOC_RUNTIME_UPDATE_2026-06.md)** (audit UI runtime 23/06/2026, captures [`audit-ui-runtime-2026-06/`](audit-ui-runtime-2026-06/))  
> - **Confirmé** : `/groups-admin/:groupId` = fourre-tout technique (exports, LLM, P0, RAG) avant la cohorte ; page la plus problématique en lisibilité.  
> - **Confirmé** : `/dashboard` oriente vers le testeur (`/group/:id`), pas l'onboarding admin (Users, Groupes admin, Config).  
> - **Nuancé** : `/users` affiche bien l'org active et permet la création **TRAINER** (SUPERADMIN) depuis 23/06 — mieux que l'audit code seul.  
> - **Confirmé** : `/tutor-prompts` et `/conduct-profiles` restent dans la navbar principale (`TesterLayout`) malgré le hub `/admin/conversation`.  
> - **Confirmé** : hub conversation — cartes posture libellées « (legacy) » alors qu’un profil global est déjà rattaché sur Demo Hugo Org (identifiant fixture `profil_conversationnel_mk1` : `_mk1` = suffixe interne, pas notion produit).  
> - **À VÉRIFIER** : parité de ces constats sur baseline Encoors (voir DOC_RUNTIME §5).
...
| `/users`, `/users/:userId` | Gestion utilisateurs (LEARNER, TUTOR, **TRAINER** si SUPERADMIN) | Admin comptes |
