...
> **Dernière mise à jour :** 2026-06-20 (post-profils globaux apprenant + audit pipes admin ↔ runtime — voir §9.1).
...
| **110** | Interfaces formateur / tuteur | `test_encadrants_role_guards`, `test_cluster4`, **`test_cluster15_*`**, **`test_learner_conversation_global_profile`** | smokes + **cluster16** + **`admin_learner_profiles`**, **`admin_conversation_pipeline`**, **`tenant_personas`** | — | — | Tuteur prod **A_VÉRIFIER** ; IFT-042 **CIBLE** ; trous admin synthèse/starter prompts |
...
| `test_learner_conversation_global_profile.py` | 10, 31, 70, 110 (profil global, résolution, legacy template, CRUD) |
...
| **`e2e/admin_learner_profiles.spec.ts`** | **D1 ORGADMIN** | Création profil global apprenant (20/06) |
| **`e2e/admin_conversation_pipeline.spec.ts`** | **D2 SUPERADMIN** | Création profil + affectation groupe ; hub legacy LLM (20/06) |
| **`e2e/tenant_personas.spec.ts`** | **Multi-tenant** | SUPERADMIN switcher, ORGADMIN scope, TUTEUR/APPRENANT isolation (**11 PASS** — campagne 20/06 archivée) |
...
Synthèse consolidée **18/06** (convergence) + **20/06** (multi-tenant, RLS, tutor-links, UI, smoke CI, **profils globaux apprenant**) : [`tests/archives/tests_hugo_2_0_2026-06-18_20.md`](tests/archives/tests_hugo_2_0_2026-06-18_20.md).
...
### 9.1 Audit pipes conversationnels (20/06 PM)

Campagne croisée admin ↔ runtime ↔ tests (local) :

| Suite | Résultat |
|-------|----------|
| `test_learner_conversation_global_profile.py` | 12 PASS |
| Pipeline convo (eval, posture, tutorprompt, guards) | 26 PASS |
| C16 + cluster15/16 backend apprenant | 32 PASS |
| Playwright profils + pipeline + tenant | 15 PASS (4 admin convo + 11 tenant) |
| Front unit + build | 25 PASS ; build OK |

**Trous admin documentés (non bloquants) :** synthèse, orchestrateur statique, starter prompts, policy eval groupe, session override profil global sans REST.
