...
| IFT-009 | Verbatim non partagé | Le verbatim non partagé reste caché aux rôles tuteur / formateur | La doctrine 2.0 et le cadrage POC le fixent ; rien dans les audits mobilisés ici ne prouve le contraire | ALIGNE | Garde-fou stable et structurant | Conserver tel quel dans la doc de domaine |
...
| IFT-017 | Timeline apprenant | Le tuteur / formateur peut consulter une timeline de progression | Le cadrage POC cite `GET dashboard/groups/{groupid}/learners/{learnerid}/timeline` | ALIGNE_DOC_PARTIEL | Point bien cadré dans le POC, mais à recouper avec produit montrable courant | Documenter comme contrat observé / POC, pas comme vérité exhaustive du runtime |
...
| IFT-026 | Sélection de posture côté interface | La cible 2.0 prévoit une sélection ou exposition bornée de posture | Le glossaire classe `PATCH .../posture` en `A_VERIFIER` dans le réel ; pas de preuve suffisante ici | A_VERIFIER | Ne pas documenter ce contrôle comme acquis côté tuteur / formateur | Marquer comme contrat cible non prouvé |
...
IFT-041 Profils d’affichage apprenant par public La cible 2.0 admet plusieurs profils d’affichage apprenant (jeunes en formation initiale, adultes en reconversion, profils ingénieur), lisant tous le même UIState backend et différant surtout par densité textuelle et langage visuel. Le rel audit montre une seule variante d’UI apprenant montrable, avec un design plutôt intermédiaire. ABSENT NOUVEAUCONTRAT Le besoin de différenciation UX est doctrinalement clair, mais aucune galerie de profils ni contrat de rendu multi-profils n’est encore documenté dans le rel. Documenter les profils d’affichage comme cible 2.0, sans les décrire comme livrés ; ouvrir un chantier de spécification UX et de mapping sur UIState.
