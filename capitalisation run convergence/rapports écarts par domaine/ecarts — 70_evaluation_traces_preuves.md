...
|-------------------------|--------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|--------------------|
| EvaluationTrace         | Trace terminale structurée d’évaluation ou de bilan, rattachée à une session et exploitable par tuteur / exports.              | Pas de modèle `EvaluationTrace` direct, mais objets réels `LearnerEvaluationRecord`, `Trace`, `TraceCriterionAssessment`. | Concept présent, mais mapping doctrine ↔ réel dispersé (`EvaluationTrace` vs `LearnerEvaluationRecord` vs `Trace`).                    | AMBIGU             |
| LearnerEvaluationRecord | Non nommé côté doctrine 2.0, mais candidat naturel pour incarner l’EvaluationTrace côté implémentation.                        | Modèle réel observé, associé au workflow d’évaluation.                                                          | Rôle réel aligné conceptuellement avec EvaluationTrace, mais nom et contrat encore peu visibles dans la documentation 2.0.             | RENOMMER_DANS_DOC  |
