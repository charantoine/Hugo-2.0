...
| CONF-07 | Tuteur sans accès libre au non-partagé | Le tuteur lit progression, blocages, traces partagées, sans accès libre au verbatim non partagé ni contrôle direct du moteur. | La cible le fixe très clairement ; le réel montre surtout des vues dérivées et non un accès brut, mais le parcours tuteur complet n’est pas intégralement démontré dans le corpus mobilisé. | vues dérivées, traces partageables, surfaces tutorales partielles | ALIGNE_DOC_PARTIEL | Le sens est bon, mais la démonstration produit reste partielle. | Clarifier dans la doc ce qui est déjà observable côté tuteur et ce qui reste cible partiellement ouverte. |
...
### 3.3 Zones à ne pas sur-affirmer
