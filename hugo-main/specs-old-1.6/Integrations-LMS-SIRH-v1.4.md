# Intégrations & Effets de bord (LMS/SIRH) — v1.4

**Document complémentaire** : `SPEC_POC_v1.4.md` (source de vérité / SoT)  
**Date** : 19 février 2026  
**Version** : 1.4 — Aligné avec SPEC v1.4
...
- Section 21 de `SPEC_POC_v1.4.md` (Intégrations & effets de bord LMS/SIRH)
...
**CPU-only souverain** :
- Pas d'API LLM externes (Mistral self-hosted uniquement)
...
- **Références mises à jour** : alignement sur `SPEC_POC_v1.4.md` (sections 3, 13, 21).
- **RLS** : clé Postgres normalisée `app.organisation_id` (exemples SQL corrigés).
- **Confidentialité-first** : partage par défaut tout à `false` + opt-in guidé explicité ; "pas de logs de contenu brut" formalisé.
- **`external_identity`** : encadrement multi-tenant (pas de lecture globale sans garde-fous, note ajoutée).
- **SCORM** : clarification (SCORM reste côté LMS, intégration Hugo via xAPI/LTI uniquement).
- **Visio** : retrait de la ligne "Visio/communication" (hors périmètre section 21 SPEC v1.4).

---

**FIN DU DOCUMENT — Intégrations & Effets de bord (LMS/SIRH) v1.4**
