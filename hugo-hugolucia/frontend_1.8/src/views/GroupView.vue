...
const canRunExports = computed(() => auth.isOrgAdminLike)

const exportProminent = computed(() => {
  if (!canRunExports.value) return false
  return String(route.query.focus || '').toLowerCase() === 'exports'
})
...
watch(loading, async (isLoading) => {
  if (!isLoading && exportProminent.value) {
    await nextTick()
    document.getElementById('group-exports-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
})
  ...
  <div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div class="d-flex gap-2">
        <template v-if="canRunExports && exportProminent">
          <button
            type="button"
    ...
    <section
      v-if="canRunExports && exportProminent"
      id="group-exports-panel"
      class="card shadow-sm mb-3"
      aria-label="Exports audit Qualiopi lite"
    >
      <div class="card-body">
    ...
    <section id="group-learners-panel" aria-label="Apprenants du groupe">
    ...
    >
      <summary class="small fw-semibold mb-0">
        Exports audit Qualiopi lite (Felix-ready)
      </summary>
      <div class="mt-3">
