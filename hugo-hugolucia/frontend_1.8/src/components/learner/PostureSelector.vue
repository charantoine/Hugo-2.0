...
const currentDescription = computed(() => postureDescription(currentCode.value))
...
async function onPostureChange(event) {
  const nextPosture = String(event.target.value || '').trim()
  ...
  if (target?.disabled) {
    ...
    event.target.value = currentCode.value
    return
  }
...
<template>
  ...
  >
    <div class="card-body py-3">
      ...
      <div
        v-if="backendWarning"
        class="alert alert-warning small mb-0 mt-3"
        role="status"
      >
        {{ backendWarning }}
      </div>
