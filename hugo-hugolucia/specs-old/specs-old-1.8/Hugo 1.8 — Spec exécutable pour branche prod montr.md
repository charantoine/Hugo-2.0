...
Cette spec définit l’implémentation de **Hugo 1.8** comme une **branche prod montrable** de l’application existante, tout en **conservant le frontend actuel comme branche testeur/debug**. Hugo 1.8 ne redéfinit pas le noyau Hugo 1.7/P0 ; il ajoute une couche produit/UX de progression visible et d’engagement léger au-dessus du runtime Django existant.[^1][^2][^3][^4]

## Sources de vérité

Ordre de priorité documentaire obligatoire :

1. `p0_description_technique_actuelle_mise_a_jour.md` : vérité du moteur actuel, de la chaîne runtime, des champs `TurnState`, de `ConversationDecision`, des règles 0/1/2 question et des priorités de décision.[^1]
2. `Hugo-1.6-Cadrage-Transformation-du-comportement.md` : doctrine de fond, invariants produit, mémoire gouvernée, confidentialité-first, repo-first, moteur piloté par état.[^4]
3. `Quel-type-dengagement-vise-t-on-en-priorite-__Ret-2.md` et `Nous-ouvrons-ici-le-chantier-Hugo-1.8-centre-sur-3.md` : cadrage 1.8 ludification, objectifs d’engagement, mécaniques autorisées/interdites, hypothèse “P0 inchangé + petit engagement engine léger + UI visible”.[^2][^5]
4. `SPEC_POC_v1.5-update-27_03_2026.md` : cadre technique plateforme, Django, TutorPrompt, API, multi-tenant, confidentialité, webapp responsive, tant qu’il n’entre pas en conflit avec P0 actuel.[^3]
...
- un `EngagementUIModel` dérivé des signaux runtime existants.[^2][^1]
...
- de leaderboard ;
- de streak central ;
- de réouverture artificielle de scène.[^4][^1][^2]
...
- la règle “un seul objectif de régulation par message”.[^3][^1][^4]
...
- partage explicite ;
- repo-first ;
- inscription dans une trajectoire future plus large que le seul cas AFEST.[^3][^4]
...
- la solution privilégiée est `P0 inchangé ou presque + petit engagement engine léger à côté de P0 + UI produit qui rend visible la progression`.[^5][^2]
...
- visibilité du comportement réel du moteur.[^1][^2]
...
- reste branché sur le même backend et les mêmes endpoints métier.[^2][^3]
...
- les mêmes règles de confidentialité, RLS et partage.[^4][^1][^3]
...
- aucune contamination du noyau tutoriel par la couche ludique.[^1][^2]


### Placement du moteur léger d’engagement

Le petit moteur d’engagement doit être placé **hors P0**, dans une couche produit/applicative. Il consomme les sorties runtime existantes et calcule un modèle UI dérivé. Il ne renvoie aucune consigne au moteur tutoriel.[^2][^1]
...
Aucun nouveau champ P0 ni nouveau champ `ConversationDecision` ne doit être introduit dans ce lot pour supporter la ludification.[^2][^1]
...
- `ui_visibility_flags`.[^2]

Ce contrat doit être stable pour le front, découplé des internals P0, mais alimenté par eux.[^1][^2]
...
Ce mapping est descriptif, non scriptant. Il représente l’état de la scène à partir des signaux réels, sans imposer un tunnel rigide.[^1][^2]
...
- préparer le message pour le tuteur.[^2][^1]
...
- badges ;
- thèmes visuels ;
- variantes sobres d’avatar Hugo.[^5][^2]
...
- styles caricaturaux ou infantilisants.[^5][^2]
...
Ces profils modulent uniquement l’expression visuelle et la fréquence des feedbacks, jamais la logique tutorale.[^1][^2]
...
- créer une route prod montrable dédiée, par exemple `/app/session/:id`, branchée sur les mêmes endpoints backend métier.[^3][^2]
...
Si le payload actuel n’est pas suffisant pour le front prod montrable, créer une projection dérivée dédiée, par exemple `GET /hugo/sessions/{session_id}/engagement-view`.[^3][^2]

Cette projection expose le `EngagementUIModel`, sans modifier le contrat central du moteur.[^3][^2]
...
- conformité avec la doctrine confidentialité-first existante.[^4][^3]

L’UI prod montrable ne doit jamais contourner les `share_*` flags existants.[^4][^3]
...
- le mini-bilan/texte tuteur est valorisé comme accomplissement de scène.[^1][^2]
...
- préemption de Hugo 1.9 longitudinal.[^4][^2][^1]
...
- wording produit final de “compétences mobilisées”.[^5][^3][^2]

<div align="center">⁂</div>

[^1]: p0_description_technique_actuelle_mise_a_jour.md

[^2]: Nous-ouvrons-ici-le-chantier-Hugo-1.8-centre-sur-3.md

[^3]: SPEC_POC_v1.5-update-27_03_2026.md

[^4]: Hugo-1.6-Cadrage-Transformation-du-comportement.md

[^5]: Quel-type-dengagement-vise-t-on-en-priorite-__Ret-2.md
