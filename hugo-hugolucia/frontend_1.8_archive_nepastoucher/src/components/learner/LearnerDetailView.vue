...
const p0DebugEnabled = computed(() => (
  frontendFeatures.frontend_mode === 'tester' && frontendFeatures.p0_debug_enabled
))
...
function canShowP0Modal(message) {
  if (!p0DebugEnabled.value) return false
...
function openP0Modal(message) {
  if (!p0DebugEnabled.value) return
  const req = resolveLlmRequestPayloadForMessage(message)
      ...
      <div v-else class="card">
        ...
        <div class="card-body">
          ...
          >
            ...
            ></div>
            <div
              v-if="p0ModalOpen && p0DebugEnabled"
              class="modal fade show d-block"
              style="z-index: 1055"
              tabindex="-1"
              role="dialog"
