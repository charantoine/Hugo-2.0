...
@pytest.mark.django_db
def test_superadmin_creates_trainer_in_target_org(superadmin_a, org_b, api_client):
    api_client.force_authenticate(user=superadmin_a)
    response = api_client.post(
        "/users/",
        {
            "username": "new_trainer_b",
    ...
    assert response.status_code == 201
    payload = response.json()
    assert payload["role"] == Role.TRAINER
    created = User.objects.get(username="new_trainer_b")
    assert created.organisation_id == org_b.id
    assert created.role == Role.TRAINER


@pytest.mark.django_db
def test_orgadmin_cannot_create_trainer(orgadmin_a, api_client):
    api_client.force_authenticate(user=orgadmin_a)
    response = api_client.post(
        "/users/",
        {
    ...
    assert response.status_code == 400
    assert "ORGADMIN can only create learner or tutor" in str(response.json())


@pytest.mark.django_db
def test_orgadmin_can_still_create_learner_and_tutor(orgadmin_a, api_client):
    api_client.force_authenticate(user=orgadmin_a)
    for username, role in (("org_learner_new", Role.LEARNER), ("org_tutor_new", Role.TUTOR)):
        response = api_client.post(
            "/users/",
            {
                "username": username,
        ...
        assert response.status_code == 201, username
        assert User.objects.get(username=username).role == role
