...
class LibraryIngestionIntegrationTests(TestCase):
    ...
    def test_create_index_link_and_rag_selects_chunks(self):
        ...
        selections = select_rag_chunks(
            ...
            teaching_plan=tp,
            turn_state=_turn_state(safety_or_quality_risk_level="high"),
    ...
    def test_trainer_library_flow_with_meta(self):
        trainer = User.objects.create_user(
            username="trainer_lib_meta",
            password="pass",
            organisation=self.organisation,
            role=Role.TRAINER,
        )
        ...
        r = self.client.post(
            reverse("document_list_create"),
            {"title": "Cours formateur", "source_text": "procedure tableau electrique " * 5, "meta": meta},
            format="json",
        )
        self.assertEqual(r.status_code, 201, r.content)
        doc_id = r.data["id"]
        self.assertEqual(r.data["meta"]["conversation_role"], "reference_course")
        self.client.post(reverse("document_index", kwargs={"document_id": doc_id}), {}, format="json")
        lib_url = reverse("group_library", kwargs={"group_id": self.group.id})
        self.client.post(lib_url, {"document_id": doc_id}, format="json")
        r_list = self.client.get(lib_url)
        self.assertEqual(r_list.status_code, 200)
        items = r_list.data.get("items") or []
        self.assertEqual(len(items), 1)
        self.assertEqual(items[0]["document_meta"]["conversation_role"], "reference_course")
